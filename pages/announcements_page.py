...
from data.constants import AnnouncementsPage

class AnnouncementsPage(BasePage):
    # Page Header Elements
    PAGE_TITLE = (By.CSS_SELECTOR, ".card__header span")
    NEW_ANNOUNCEMENT_BUTTON = (By.CSS_SELECTOR, "a[href='/admin/announcements/new']")

    # Search and Filter Elements
    SEARCH_INPUT = (By.ID, "headline")
    ...
    ANNOUNCEMENT_HEADLINE = (By.CSS_SELECTOR, f"td:nth-child({AnnouncementsPage.TableColumns.HEADLINE})")
    STATUS = (By.CSS_SELECTOR, f"td:nth-child({AnnouncementsPage.TableColumns.STATUS}) .badge")
    ACTIONS = (By.CSS_SELECTOR, f"td:nth-child({AnnouncementsPage.TableColumns.ACTION})")

    # Update column headers
    HEADLINE_HEADER = (By.XPATH, f"//button[@data-button-type='{AnnouncementsPage.TableColumns.HEADLINE}']")
    STATUS_HEADER = (By.XPATH, f"//button[@data-button-type='{AnnouncementsPage.TableColumns.STATUS}']")
    ...
    def get_announcement_details(self, row):
        """Get details for an announcement row"""
        return {
            'headline': row.find_element(*self.ANNOUNCEMENT_HEADLINE).text,
            'status': row.find_element(*self.STATUS).text
        }

    def get_all_announcements(self):
        """Get all announcements from current page"""
        announcements = []
        rows = self.find_elements(self.TABLE_ROWS)
        for row in rows:
            announcements.append(self.get_announcement_details(row))
        return announcements

    def search_announcement(self, name):
        """Search for a announcement by name with improved waits"""
        try:
            self.logger.info(f"Searching for announcement: {name}")
            ...
            def verify_search_result():
                rows = self.find_elements(self.TABLE_ROWS)
                for row in rows:
                    if row.find_element(*self.ANNOUNCEMENT_HEADLINE).text.strip() == name:
                        return True
                return False

            # Wait up to 10 seconds for announcement to appear
            long_wait.until(lambda d: verify_search_result())
            self.logger.info(f"Announcement '{name}' found in search results")
    ...
    def sort_by(self, order):
        """Sort announcements by given order using TomSelect dropdown"""
        try:
    ...
    def filter_by_status(self, status):
        """Filter announcements by status using TomSelect dropdown"""
        try:
    ...
    def click_new_announcement(self):
        """Click new announcement button and wait for navigation"""
        try:
            self.logger.info("Attempting to navigate to new announcement page")
            ...
            button = self.wait.until(EC.element_to_be_clickable(self.NEW_ANNOUNCEMENT_BUTTON))
            ...
            # Wait for add announcement page to load
            ...
            self.logger.error(f"Failed to navigate to new announcement page: {str(e)}")
            return self

    def edit_announcement(self, name):
        """Edit announcement by name"""
        rows = self.find_elements(self.TABLE_ROWS)
        for row in rows:
            if row.find_element(*self.ANNOUNCEMENT_HEADLINE).text == name:
    ...
    def delete_announcement(self, name):
        """Delete an announcement by name with error message check"""
        try:
            self.logger.info(f"Attempting to delete announcement: {name}")

            # Find and click delete button for announcement
            found = False
            rows = self.find_elements(self.TABLE_ROWS)
            for row in rows:
                if row.find_element(*self.ANNOUNCEMENT_HEADLINE).text.strip() == name:
            ...
            if not found:
                self.logger.error(f"Announcement '{name}' not found")
                return False
            ...
            # Wait a moment and verify announcement is gone
            self.driver.implicitly_wait(2)
            remaining_rows = self.find_elements(self.TABLE_ROWS)
            for row in remaining_rows:
                if row.find_element(*self.ANNOUNCEMENT_HEADLINE).text.strip() == name:
                    self.logger.error(f"Announcement '{name}' still exists after deletion")
                    return False

            self.logger.info(f"Announcement '{name}' successfully deleted")
            return True

        except Exception as e:
            self.logger.error(f"Failed to delete announcement: {str(e)}")
            return False

    def cancel_delete(self, name):
        """Cancel announcement deletion with improved modal handling"""
        try:
            ...
            rows = self.find_elements(self.TABLE_ROWS)
            still_exists = any(
                row.find_element(*self.ANNOUNCEMENT_HEADLINE).text.strip() == name
                for row in rows
            )

            if not still_exists:
                self.logger.error("Announcement disappeared after cancellation")
                return False

            self.logger.info(f"Successfully cancelled deletion of announcement '{name}'")
            return True
    ...
    def navigate_to_page(self, direction):
        ...
        try:
            ...
            # Get current announcements before navigation
            current_announcements = self.get_all_announcements()
